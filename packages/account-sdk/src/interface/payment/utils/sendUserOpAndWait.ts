  ...
  try {
    ...
    if (completedOp.status === 'failed') {
      throw new PaymentError(
        `${context} user operation was rejected on-chain (hash: ${userOpResult.userOpHash}). ` +
          'This may be due to insufficient gas, invalid calldata, or a contract revert.',
        'USER_OP_FAILED',
        true,
      );
    }
    ...
    if (!transactionHash) {
      throw new PaymentError(
        `${context} user operation completed with status "${completedOp.status}" ` +
          `but no transaction hash was returned (hash: ${userOpResult.userOpHash}). ` +
    ...
    throw new PaymentError(
      `Failed to execute ${context} transaction: ${errorMessage}. ` +
        'Verify the smart wallet has sufficient funds and the correct network is selected.',
      'EXECUTION_FAILED',
      true,
    );
  }
